...
class WorkerSettings:
    ...
    # timeout=1800은 bizinfo 100건 × 평균 3 첨부 worst-case 흡수 (Q95). functions 리스트의
    # `ingest_postings`는 default(300s) 유지 — manual enqueue는 Phase 1까지 발생 안 함.
    # max_tries=1 (default): cron 자체는 1회. 재시도는 tenacity가 attachment/HTTP 단위 처리
    # (`core/http.py`). run_at_startup=False (default): rolling deploy double-fire 방지 —
    # dev 검증은 `await ingest_postings({})` 직접 호출 또는 ARQ CLI manual enqueue로 (cron 발화 대기 불요).
    cron_jobs: ClassVar[list[Any]] = [
        cron(ingest_postings, hour={17}, minute=0, timeout=1800.0),
    ]
