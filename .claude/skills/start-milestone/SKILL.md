...
After answers received: record the resolved decisions in plan.md "결정 확정" 섹션 for traceability (both Batch A macro and Batch B micro decisions — Q-numbering 단일 시퀀스 audit trail). Then update plan.md scope if needed (Batch A) or proceed to code (Batch B).
