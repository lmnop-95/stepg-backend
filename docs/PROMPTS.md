...
**M4 main 코드 의존성 (critical — M4 main PR 첫 commit 필수)**:
- **M1 `AUDIT_ACTIONS` enum 확장 migration**: `STAGE2_INVALID_TAG` / `STAGE2_INVALID_FIELD` 두 신규 action 추가 + `_AUDIT_ACTION_CHECK_SQL` CheckConstraint 동시 갱신. §6 step 3 / §8 일괄 헤더 / §8.4 golden 예제 모두 본 enum 확장 전제. 미반영 시 Stage 2 invalid 로깅 / golden test 모두 `IntegrityError: action_allowed CHECK constraint violation` 발생.
- **prompt builder inline expansion 로직**: §1 의 `eligibility` 자리에 §1.1 의 18 필드 properties / required 전체를 inline expand 하는 SDK 호출 코드. Anthropic tool input_schema `$ref`/`$defs` 미지원이라 §1.1 변경 = builder 코드 갱신 필수 (cache invalidation 만으로 부족 — runtime 에 stale schema → LLM validation mismatch).
- **placeholder substitution 룰** (§2.1 강제 사항): `{TAXONOMY_TREE}` / `{TAXONOMY_BOUNDARY}` 두 키 만 substitute, 그 외 `{POSTING_*}` literal 은 그대로 보존. naive `.format(**all)` 금지.
...
**substitution 대상 룰 (강제)**: M4 main 의 prompt 빌더는 §3 system prompt 본문에서 `{TAXONOMY_TREE}` / `{TAXONOMY_BOUNDARY}` **두 placeholder 만** substitute **해야 합니다 (MUST)**. 구현 방식:
- **Option A (권장)**: `str.replace()` 명시 호출 — `system_text.replace("{TAXONOMY_TREE}", cached_tree).replace("{TAXONOMY_BOUNDARY}", cached_boundary)`. 본 양식이 `{POSTING_BODY}` 등 documentation literal 그대로 보존 (LLM cross-ref 신호 유지) + KeyError 차단.
- **Option B**: §3 본문 작성 시 documentation literal 을 `{{POSTING_BODY}}` (doubled brace) 로 escape → `.format(TAXONOMY_TREE=..., TAXONOMY_BOUNDARY=...)` 호출 후 `.format()` 의 자동 single brace 복원 활용.
- **금지 (MUST NOT)**: `system_text.format(**all_placeholders)` 같은 5 placeholder 일괄 substitute — POSTING_* placeholder 들이 system prompt 안에서 literal 사라져 (a) LLM cross-ref 신호 손실, (b) cache_control 분리 의도 위배 (POSTING_* 는 user message 동적 바인딩 영역, system 캐시 영역 X). cache invalidation 빈도 폭증.

`{POSTING_BODY}` / `{ATTACHMENT_TEXT}` / `{POSTING_META}` literal 은 §3 본문 안 documentation reference — LLM 에게 "user message 에 이런 placeholder 가 채워져 들어옵니다" 안내 신호로 그대로 박힘. 동적 바인딩은 §4 user prompt 양식 SoT.
...
## 제약
- **신뢰도 임계 세 가지 (독립 적용 — 어느 zone 으로 emit 할지 결정 시 참조)**:
  1. **0.7 임계 = Stage 3 분기 트리거** (`ARCHITECTURE.md §5` line 239-240 SoT):
     - `tag_confidence_per_id` 의 `< 0.7` 값 (모름 / 모호 / 추론 zone path) 이 **3개 이상** 누적 → needs_review 자동 분기.
     - `field_confidence_scores` 의 `< 0.7` 값 (eligibility 필드 신뢰도) 이 **3개 이상** 누적 → needs_review 자동 분기.
  2. **0.5 임계 = fabricate guard rail + taxonomy match doubt** (두 의도 share, invalid <5% 목표 우선 — recall 손실 < precision 보장):
     - **(a) 본문 외 정보 추측 금지**: 공고 본문에 명시되지 않은 정보를 외부 domain prior 로 추측 시 **0.5 미만** (모호 또는 모름 zone) 으로 강제 emit. 예: 인증 요건 미명시 시 "정부 공고 일반적으로 벤처기업 우대" 같은 추측 금지 → `certifications_preferred=null` + 신뢰도 모호·모름 zone.
     - **(b) 택소노미 정확 일치 실패 시 보수적 emit**: 100 노드 트리 안 정확 일치 path 없고 가장 가까운 path 선택 시 — 강한 단서 + 부분일치 = `[0.5, 0.7)` 추론 zone, 약한 단서 = `[0.3, 0.5)` 모호 zone (단서 강도 별 분기, §6 step 2 정합).
  3. **0.3 임계 = 모호↔모름 zone 경계** — `[0.3, 0.5)` (모호) ↔ `< 0.3` (모름) 자기평가 분기점.

  세 임계 별도 의도: 0.7 = review queue 트리거 / 0.5 = fabricate guard + taxonomy doubt / 0.3 = 자기평가 zone 경계. 임계 reuse 시 의도 혼동 차단.
...
분포 = golden 4종 (분야 3 — AI / 제조·소재 / 콘텐츠 + 모호 multi-tag 1, real Anthropic API spot check) + unit test 1종 cross-ref (§8.4 invalid 로깅, LLM 호출 X — F4 (iii) M4.4 commit 1 적용). 분야 선택 기준 = `TAXONOMY.md §6.3(a)` 5분야 (AI/데이터, 바이오, 제조/소재, 콘텐츠/디자인, 이커머스) 중 **multi-tag dense + §5.1 (a) boundary 룰 검증 cover 분야 우선** (cross-axis 신호 풍부 — AI ↔ 제조 / 제조 ↔ 클린텍 / 콘텐츠 ↔ 수출). 바이오·이커머스는 단일 axis 경향 + M6 150쌍 eval 세트 cover (본 §8 외부 위임). 입력 = bizinfo 실 데이터 3건 (source_id + content_hash 인용, DB fetch reproducible) + 합성 1건 (§8.5) + unit test 1건 (§8.4 cross-ref — `tests/unit/extraction/test_stage2.py`). expected output = 핵심 필드 (eligibility 6 + tags 5 + summary) — 풀스펙 `ExtractedPostingData` JSON 은 M6 150쌍 eval set SoT (`ARCHITECTURE.md §9` line 411-413 M6 평가 세트 + `docs/eval/guide.md`). 신뢰도 표기 = zone 단위 (값 X) — calibration robust.

**채점 임계 (§8.1-§8.5 양식 통일)**: 자동 채점 = (a) multi-tag 필드 (`field_of_work_tag_ids`) tag set Jaccard ≥ 0.75 (부분 일치 허용 — boundary 룰 적용 시 우선 path 1 + cross-tag 1-2 기준), (b) single-value 필드 (`corporate_types_allowed`, `funding_uses`, `location_required_sido` 단건) 정확 일치, (c) 배열 필드 ≥ N개 emit (각 §8.x 별 N 명시). 큐레이터 채점 = summary 자연 reading (200자 이내, 핵심 신호 cover) + boundary 룰 적용 일관성. **Note**: 본 임계는 M6 eval guide (`docs/eval/guide.md` — **forward reference, M6 PR 에서 추가 예정**) 의 SoT 와 일치 예정 — dual SoT 회피. 본 PR 시점 = guide 미작성, 본 §8 임계가 interim SoT.
...
(F21 적용: §8.4 row 표에서 제거 — 표 axis = LLM 처리 케이스 (easy/medium/hard) 한정. §8.4 unit test cross-ref 는 본 §8 헤더 + § 8.4 본문 + §8.6 cross-ref 로 표시.)
...
### 8.4 invalid trigger (unit test cross-ref)

**source**: `tests/unit/extraction/test_stage2.py` (F4 (iii) 적용 — M4.4 commit 1, real API + LLM 환각 의존 → unit test 분리, cost $0 + 재현성 ↑).

**검증 본체 (Stage 2 unit test)**:
- (a) **path-exact match**: canonical path 입력 → `Stage2Result.extracted.field_of_work_tag_ids` valid, `audit_rows` empty
- (b) **alias-list match**: alias 입력 → canonical remap, `audit_rows` empty
- (c) **invalid (path + alias miss)**: unknown path (`tech.quantum.computing`) → invalid drop + `STAGE2_INVALID_TAG` audit row 1건 (`Stage2Result.audit_rows[0]['action']`)

**채점**: `validate_stage1_output(posting_id, raw)` 호출 후 `Stage2Result.audit_rows` (return value, DB insert X) 검증. `_remap_tags` 의 normalize → valid_paths → alias_index → invalid drop 4-step 양식 (`PROMPTS.md §6` step 1-3) 본질 cover. LLM 호출 0 → re-runnable + deterministic.

**과거 양식 (M4.3 이전)**: real Anthropic API + 합성 본문 (양자컴퓨팅 R&D) → LLM 환각 강제. M4.3 PR.md L51-67 검증 시 Sonnet 4.6 환각 회피 (good behavior) → 항상 fail (regression test 역할 X). 본 § = M4.4 commit 1 갱신 (F4 (iii) Reviewer follow-up 결정).
...
본 §8 4종 (golden) + §8.4 unit test cross-ref = M4 main 코드 schema-level spot check (tool schema · alias remap · invalid 로깅 · §5.1 boundary 룰 · Stage 3 분기). M4 정량 (자동승인 70%+ / invalid <5% / low-conf <2개) 운영 검증은 **M6 평가 세트 (5 personas × 30 postings = 150쌍)** SoT — `ARCHITECTURE.md §9` line 411-413 + `docs/eval/guide.md` (**forward reference, M6 PR 에서 추가 예정**). M4 main 코드 commit 5 또는 별 PR 에서 M6 평가 세트 cross-ref + 정량 측정.
...
- **PROMPTS.md §1.1 EligibilityRules nested schema 변경** → tool desc cache miss (§1 의 `eligibility` 자리에 §1.1 inline expand 박는 양식 — Anthropic `$ref`/`$defs` 미지원이라 §1.1 변경 = tool desc 본문 재구성 = 새 캐시 키. dependency chain: `ARCHITECTURE.md §4.1` SoT → §1.1 mirror → tool desc cache). **추가 조치**: M4 main 의 prompt builder 코드 (SDK 호출 측) 의 inline expansion 로직도 동기화 갱신 필수 — cache invalidation 만으로 부족, runtime 에 stale schema 가 LLM 에 전달되면 validation mismatch (§0 "M4 main 코드 의존성" sub-section 참조).
...
- **golden 예제 자동 회귀 테스트** — 본 §8 4종 (golden) + §8.4 unit test cross-ref + M6 150쌍 → CI 통합 (현 Phase 1 = 수동 spot check).
- **structlog + Sentry** 도입 시 invalid 로깅 mechanism 재검토 (`ExtractionAuditLog` row + structlog 분리).
- **hot reload / blue-green deployment** — 현 Phase 1 의 전체 앱 재시작 요구 사항 완화. `cache_control: ephemeral` 단일 블록 특성상 부분 갱신 X 라 prompt tuning 시 전체 재시작 → M2 수집 → M4 추출 파이프라인 일시 중단 위험. 옵션: (a) prompt 변경 시 새 cache_control 블록을 별도 인스턴스에서 warm-up → traffic 전환 (zero-downtime blue-green), (b) Anthropic API cache TTL 활용한 gradual rollout (cache 만료 후 자동 갱신). prompt versioning 위 항목과 함께 도입 검토.
