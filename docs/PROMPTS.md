...
본 §8 5종 = M4 main 코드 schema-level spot check (tool schema · alias remap · invalid 로깅 · §5.1 boundary 룰 · Stage 3 분기). M4 정량 (자동승인 70%+ / invalid <5% / low-conf <2개) 운영 검증은 **M6 평가 세트 (5 personas × 30 postings = 150쌍)** SoT — `ARCHITECTURE.md §9` line 411-413 + `docs/eval/guide.md` (**forward reference, M6 PR 에서 추가 예정**). M4 main 코드 commit 5 또는 별 PR 에서 M6 평가 세트 cross-ref + 정량 측정.
