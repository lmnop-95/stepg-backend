...
| 파일 파싱 | HWPX `python-hwpx` (M3에서 `pyhwpx`→교체. `pyhwpx`는 Windows COM + 한글 Office 의존이라 cross-platform 불가. 설치 LICENSE = "Custom Non-Commercial" / GitHub main = Apache 2.0 분기 — Phase 1.5 SaaS 출시 전 재검토), PDF `pdfplumber`(+ `easyocr` fallback), DOCX `python-docx`. **HWP 레거시 미지원** |
...
- **Audit log immutability DB-level 강화** — `extraction_audit_logs`에 REVOKE UPDATE/DELETE + DB role 분리 (Phase 1은 service convention만)
- **`extraction_audit_logs.actor_user_id` NULL 의미 분리** — 현재 NULL이 (a) `ON DELETE SET NULL`로 잃은 actor와 (b) `AUTO_APPROVE` 시스템 actor 두 의미를 가짐. M9 admin UI에서 "[deleted user]" vs "system" distinguish 필요해지면 `is_system: Boolean` 컬럼 또는 sentinel system user row 도입 (Phase 1은 `WHERE action='AUTO_APPROVE'` 필터로 우회).
- **`TimestampMixin.updated_at` 서버 측 onupdate 트리거** — Phase 1은 ORM session의 `onupdate=lambda` 만 fire. raw SQL `UPDATE`(M2 이후 마이그레이션·cron raw upsert 등) 도입 시 BEFORE UPDATE 트리거(`NEW.updated_at = now()`) 추가하여 ORM-bypass 회귀 방지.
- **`python-hwpx` LICENSE 재검토** — M3에서 `pyhwpx`(Windows COM 의존) → `python-hwpx`로 교체. PyPI v2.9.0 설치본 LICENSE = `LicenseRef-python-hwpx-NonCommercial` (GitHub main 은 Apache 2.0 — 분기). SaaS 출시 전 maintainer 협상 / Apache 2.0 PyPI release 대기 / 자체 HWPX 파서 (lxml + ZipFile + namespace iter) 전환 중 택1.
- **`torch` CPU-only wheel 분리** — M3 commit 3에서 `easyocr` 추가가 `torch 2.11.0` universal wheel(~2.5GB Linux x86_64, CUDA runtime 포함) + ML stack 8개를 끌어옴. Q9 결정 "easyocr CPU mode 강제"와 무관하게 wheel 자체는 GPU. Phase 1.5 Docker 운영 시점에 `[tool.uv.sources] torch = { index = "https://download.pytorch.org/whl/cpu" }` 또는 `--index-url` 명시로 ~200MB cpu-only wheel 마이그레이션 (image size 90% 감소).
...
- **한국어 형태소 FTS** (mecab-ko-dic / PGroonga 등. M1은 `simple` config로 시작, 도입 시 도커 베이스 + text search config 교체 마이그레이션 동반)

**사용자/마케팅**:
- **클라우드 파일 저장소** (R2 또는 S3)
- **Kakao + Naver 로그인**
- **FAST 점수 도구** (비가입 방문자 리드 수집)
- **email 대소문자 비구분 유일성** (M5 NextAuth 시점 결정) — Auth.js Email/매직링크 Provider는 기본 lowercase 처리이지만 Google OAuth provider 대소문자 보장 미보장. 도입 옵션 3종: (a) functional unique index `UNIQUE(lower(email))`, (b) `citext` 타입 컬럼 (extension 추가), (c) app-level normalize (`email.strip().lower()`). M5 시점이 데이터 정정 비용 최저 (실 사용자 0).
